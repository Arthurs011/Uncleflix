...
const Card = ({ item, type, progress }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative flex-shrink-0 w-[30vw] min-w-[100px] max-w-[140px] sm:w-32 sm:max-w-none md:w-40 lg:w-44 xl:w-48 cursor-pointer touch-manipulation"
    >
      <Link to={`/watch/${itemType}/${id}`} className="block" aria-label={`Play ${title}`}>
        <div className="relative overflow-hidden rounded-xl aspect-[2/3] shadow-md transition-all duration-300 group-hover:shadow-[0_0_18px_rgba(34,211,238,0.3)] group-hover:border-cyan-400/40 border border-transparent">
          {poster ? (
            <>
              <img
                src={poster}
          ...
          {/* Play button — visible on hover (desktop) or always (mobile tap) */}
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-250">
            <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/90 flex items-center justify-center shadow-xl scale-80 group-hover:scale-100 transition-transform duration-250">
              <Play size={16} fill="#0B0F1A" className="text-primary ml-0.5" />
            </div>
          </div>

          {/* Title on hover */}
          <div className="absolute bottom-0 left-0 right-0 p-2 translate-y-1 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-250">
